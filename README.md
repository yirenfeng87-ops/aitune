...
### 5.6 内容模板/场景
- 预设常用场景：通用写作、小红书、公众号、知乎问答、微博快讯、营销文案、产品描述、邮件回复、论文摘要
- 每个场景包含预设角色、语气、输出格式
...
- 导出：前端使用 Blob 生成 .txt 下载；Word 后续集成
...
### 6.5 限流降级与兜底（容错）
- 非流式：当远程模型返回 429/5xx 时，自动切换备用免费模型；针对 429 追加 1.5 秒延迟后再重试一次
- 流式：当流式返回 429/5xx 时，自动切换备用免费模型流式；若仍受限，再延迟重试一次；若仍失败，转为一次性兜底输出
- 本地兜底内容：按语言（中文/English/日本語）生成结构化可用内容，包含“标题候选/内容大纲/正文/结尾总结”，首行明确提示：
  - 中文：提示：当前内容为生成失败后的兜底内容（本地生成），仅供参考
  - English：Notice: This is locally generated fallback content due to remote generation failure or rate limit. For reference only.
  - 日本語：注意：これは生成失敗・レート制限時のローカルフォールバックです。参考用。
- 输出方式：非流式场景以 JSON 返回；流式场景以 SSE 单次事件推送兜底文本，保证前端无“空结果”
- 前端交互（可选）：识别兜底提示，展示醒目标签，建议用户稍后重试以获取完整结果
